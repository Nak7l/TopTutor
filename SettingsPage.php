...
<body class="bg">
    ...
    <div class="settings-container">
        ...
        <div id="profile" class="tab-content">
            ...
            <h2 class="fw-bold text-color"><?php echo $student['name'] ?></h2>
            <form id="updateProfileForm">
                ...
                <div class="mb-3">
                    <input type="hidden" class="form-control" name="student_id" value="<?php echo $student['student_id'] ?>" required>
                </div>
                <div class="mb-3">
        ...
        <div id="email-phone" class="tab-content text-color" style="display: none;">
            ...
            <form>
                <div class="mb-3">
                    <input type="email" class="form-control" name="email" value="<?php echo $student['email'] ?>" placeholder="New Email" required>
                </div>
                <div class="mb-3">
                    <input type="number" class="form-control" name="phone" value="<?php echo $student['phone'] ?>" placeholder="New Phone" required>
                </div>
                <button type="submit" class="btn button w-100">Save Changes</button>
            </form>
        </div>
        ...
        <div id="other-details" class="tab-content text-color" style="display: none;">
            ...
            <form id="other-details">
                <div class="mb-3"><input type="text" class="form-control" name="city" value="<?php echo $student['city'] ?>" placeholder="City"></div>
                <div class="mb-3">
                    <select class="form-control" name="class" value="<?php echo $student['class'] ?>">
        ...
        <div id="delete-account" class="tab-content" style="display: none;">
            ...
            if (isset($_POST['updateProfile'])) {
                ...
                $folder = "admin/uploads/users_img/students/";
                $imageFileType = pathinfo($folder . basename($profile), PATHINFO_EXTENSION);

                if (!empty($profile)) {
                    move_uploaded_file($temp_name, $folder . $profile);
                } else {
                    $query = "SELECT profile FROM students WHERE student_id = '$student_id'";
                    $result = mysqli_query($db, $query);
                    $row = mysqli_fetch_assoc($result);
                    $profile = $row['profile'];
                }
            
                $query = "UPDATE students SET name='$name', profile='$profile' WHERE student_id='$student_id'";
